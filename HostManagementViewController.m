...
-(IBAction)addHost:(id)sender{
    ...
    [delegateVC presentModalViewController:hostController animated:YES];
}
...
/*    NSUInteger nrGateways = [[AppDelegate instance].veraConnector kodiGateways].count;
    if (nrGateways == 0 && !tableView.editing) {
        return 1; 
    }
    return nrGateways;
 */
...
//    if (nrGateways == 0){
        ...
        KodiGateway *kgw = [[[AppDelegate instance].veraConnector kodiGateways] objectAtIndex:indexPath.row];
        
        [cellLabel setText:kgw.deviceName];
        [cellIP setText:kgw.localAddress];
...
-(void)selectServerAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // TODO: Hanlde selection of server here
    /*
...
/*	if (editingStyle == UITableViewCellEditingStyleDelete){
        ...
        [[AppDelegate instance].veraConnector.gateways removeObjectForKey:kgw.id];
...
/*    NSUInteger nrGateways = [[AppDelegate instance].veraConnector kodiGateways].count;
    ...
    if (nrGateways == 0 && !serverListTableView.editing) return;
...
/*    NSUInteger nrGateways = [[AppDelegate instance].veraConnector kodiGateways].count;
    ...
    if (lpgr.state == UIGestureRecognizerStateBegan){
        ...
        if (indexPath != nil && indexPath.row < nrGateways){
            [self modifyHost:indexPath];
        }
    }*/
}
...
-(void)selectIndex:(NSIndexPath *)selection reloadData:(BOOL)reload{
    if (reload){
        ...
        // Test server connections as well
/*        for (KodiGateway *gw in [AppDelegate instance].veraConnector.kodiGateways)
        {
            [gw testConnectionForDelegate:self];
        }*/
    }
    else if (selection){
...
- (void)viewDidLoad{
    ...
    [supportedVersionLabel setText:NSLocalizedString(@"Welcome to Ctrlable Media Center", nil)];
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")){
...
*/
     ...
     */
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(connectionTestSuccess:)
                                                 name: @"tcpJSONRPCConnectionOpened"
                                               object: nil];
}

- (void)connectionTestSuccess:(NSNotification *)note
{
/*    NSLog(@"Connection on:%@", ((KodiGateway *)note.object).deviceName);
    NSUInteger i = [[AppDelegate instance].veraConnector.kodiGateways indexOfObject:note.object];
    if (i != NSNotFound)
    {
        UITableViewCell *cell  = [serverListTableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
        [(UIImageView *)[cell viewWithTag:1] setImage:[UIImage imageNamed:@"connection_on"]];
    }*/
}
